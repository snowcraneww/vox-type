...
pub fn save_baidu_asr_secret_key_to_user_env(
    ...
    if normalized_key.is_empty() {
        return Err(VoxError::Config("百度 ASR Secret Key 不能为空。".to_string()));
    }
...
fn baidu_config_from_existing_config(existing_config: CloudAsrConfig) -> CloudAsrConfig {
    if existing_config.provider.eq_ignore_ascii_case(BAIDU_PROVIDER) {
...
fn minimax_status_from_config(config: CloudAsrConfig, api_key: Option<String>) -> CloudAsrConfigStatus {
    ...
    CloudAsrConfigStatus {
        config,
        api_key_configured,
        api_key_source: if api_key_configured { "env:MINIMAX_API_KEY" } else { "missing" }.to_string(),
        api_key_preview,
...
fn baidu_status_from_config(
    ...
    let has_base_url = has_value(config.base_url.as_ref());
    let has_supported_endpoint = config.base_url.as_ref().is_some_and(|value| value.trim().trim_end_matches('/').ends_with("/server_api"));
    let has_model = has_value(config.model.as_ref());
    ...
    let ready = api_key_configured && secret_key_configured && has_base_url && has_supported_endpoint && has_model && has_cuid && has_format && has_sample_rate;
    let message = if ready {
    ...
    CloudAsrConfigStatus {
        config,
        api_key_configured,
        api_key_source: if api_key_configured { "env:BAIDU_ASR_API_KEY" } else { "missing" }.to_string(),
        api_key_preview,
        secret_key_configured,
        secret_key_source: if secret_key_configured { "env:BAIDU_ASR_SECRET_KEY" } else { "missing" }.to_string(),
        secret_key_preview,
        ready,
        message,
    }
...
fn normalize_config(config: CloudAsrConfig) -> CloudAsrConfig {
    let provider = trim_or_default(Some(config.provider), MINIMAX_PROVIDER).to_lowercase();
    if provider == BAIDU_PROVIDER {
        return CloudAsrConfig {
            provider,
            group_id: trim_optional(config.group_id),
            base_url: trim_optional(config.base_url).or_else(|| Some(DEFAULT_BAIDU_ENDPOINT.to_string())),
            ...
            baidu_cuid: trim_optional(config.baidu_cuid).or_else(|| Some(DEFAULT_BAIDU_CUID.to_string())),
            baidu_format: trim_optional(config.baidu_format).or_else(|| Some(DEFAULT_BAIDU_FORMAT.to_string())),
    ...
    CloudAsrConfig {
        provider: MINIMAX_PROVIDER.to_string(),
        group_id: trim_optional(config.group_id),
        base_url: trim_optional(config.base_url).or_else(|| Some(DEFAULT_MINIMAX_BASE_URL.to_string())),
...
fn write_user_environment_variable(name: &str, value: &str) -> Result<(), VoxError> {
    ...
    extern "system" {
        fn RegCreateKeyExW(h_key: Hkey, lp_sub_key: *const u16, reserved: u32, lp_class: *mut u16, dw_options: u32, sam_desired: u32, lp_security_attributes: *mut c_void, phk_result: *mut Hkey, lpdw_disposition: *mut u32) -> i32;
        fn RegSetValueExW(h_key: Hkey, lp_value_name: *const u16, reserved: u32, dw_type: u32, lp_data: *const u8, cb_data: u32) -> i32;
        fn RegCloseKey(h_key: Hkey) -> i32;
        fn SendMessageTimeoutW(h_wnd: *mut c_void, msg: u32, w_param: usize, l_param: isize, fu_flags: u32, u_timeout: u32, lpdw_result: *mut usize) -> usize;
    }
    ...
    let create_result = unsafe { RegCreateKeyExW(HKEY_CURRENT_USER, sub_key.as_ptr(), 0, ptr::null_mut(), REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, ptr::null_mut(), &mut key, ptr::null_mut()) };
    if create_result != ERROR_SUCCESS {
        return Err(VoxError::Config(format!("打开用户环境变量注册表失败：Windows error {create_result}")));
    }

    let set_result = unsafe { RegSetValueExW(key, value_name.as_ptr(), 0, REG_SZ, value_data.as_ptr().cast::<u8>(), (value_data.len() * std::mem::size_of::<u16>()) as u32) };
    let _ = unsafe { RegCloseKey(key) };
    if set_result != ERROR_SUCCESS {
        return Err(VoxError::Config(format!("写入用户环境变量失败：Windows error {set_result}")));
    }

    let environment = wide("Environment");
    let _ = unsafe { SendMessageTimeoutW(HWND_BROADCAST, WM_SETTINGCHANGE, 0, environment.as_ptr() as isize, SMTO_ABORTIFHUNG, 5000, ptr::null_mut()) };
    Ok(())
}
...
fn api_key_from_user_environment_registry(name: &str) -> Option<String> {
    ...
    extern "system" {
        fn RegOpenKeyExW(h_key: Hkey, lp_sub_key: *const u16, ul_options: u32, sam_desired: u32, phk_result: *mut Hkey) -> i32;
        fn RegQueryValueExW(h_key: Hkey, lp_value_name: *const u16, lp_reserved: *mut u32, lp_type: *mut u32, lp_data: *mut u8, lpcb_data: *mut u32) -> i32;
        fn RegCloseKey(h_key: Hkey) -> i32;
    }
    ...
    let value_name = wide(name);
    let mut key: Hkey = ptr::null_mut();
    let open_result = unsafe { RegOpenKeyExW(HKEY_CURRENT_USER, sub_key.as_ptr(), 0, KEY_QUERY_VALUE, &mut key) };
    ...
    let size_result = unsafe { RegQueryValueExW(key, value_name.as_ptr(), ptr::null_mut(), &mut value_type, ptr::null_mut(), &mut byte_len) };
    ...
    let query_result = unsafe { RegQueryValueExW(key, value_name.as_ptr(), ptr::null_mut(), &mut value_type, buffer.as_mut_ptr().cast::<u8>(), &mut byte_len) };
    let _ = unsafe { RegCloseKey(key) };
...
mod tests {
    ...
    fn baidu_provider_is_ready_with_api_key_and_required_fields() {
        ...
        let status = save_cloud_asr_config_with_provider_keys(
            ...
            ProviderApiKeys { minimax_api_key: None, baidu_asr_api_key: Some("baidu-api-key".to_string()), baidu_asr_secret_key: Some("baidu-secret-key".to_string()) },
        ).unwrap();
    ...
    fn baidu_provider_defaults_to_short_speech_rest_fields() {
        let normalized = normalize_config(CloudAsrConfig {
            provider: "baidu".to_string(),
            group_id: None,
            base_url: None,
            model: None,
        ...
        assert_eq!(normalized.base_url, Some("http://vop.baidu.com/server_api".to_string()));
    ...
    fn saving_baidu_api_key_switches_minimax_disk_config_to_baidu_status() {
        ...
        save_cloud_asr_config_with_provider_keys(
            ...
            ProviderApiKeys { minimax_api_key: None, baidu_asr_api_key: None, baidu_asr_secret_key: None },
        ).unwrap();
    ...
    fn baidu_provider_requires_secret_key_for_access_token_auth() {
        let status = save_cloud_asr_config_with_provider_keys(
            ...
            ProviderApiKeys { minimax_api_key: None, baidu_asr_api_key: Some("baidu-api-key".to_string()), baidu_asr_secret_key: None },
        ).unwrap();
