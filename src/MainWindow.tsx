...
function formatInsertion(record: TranscriptRecord) {
  const insertion = record.insertion;
  if (!insertion) return null;
  if (insertion.fallbackUsed || insertion.requestedStrategy !== insertion.actualStrategy) {
    return `\u4e0a\u5c4f ${insertion.requestedStrategy} -> ${insertion.actualStrategy}`;
  }
  return `\u4e0a\u5c4f ${insertion.actualStrategy}`;
}
