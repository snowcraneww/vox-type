...
## 2026-06-02 recording overlay capsule truncation regression

### Symptom

Maintainer reported that the recording capsule overlay again showed multiple clipped/truncated edges during recording.

### Impact

The daily dictation overlay looked visibly broken even though ASR and hotkey behavior were unrelated. The issue affected both possible rendering paths: native Win32 overlay and WebView fallback.

### Root Cause

The previous overlay clipping fix only added a small vertical buffer by moving from 120 x 32 to 120 x 36. The visible capsule, stroke, glow and antialiasing were still effectively drawn against the viewport/window edge. Native layered-window rendering and WebView transparent windows can both clip semi-transparent edge pixels when the drawing has no horizontal and vertical safety margin.

### Fix

The overlay viewport/window is now 132 x 44 for both native Win32 and WebView fallback. The visible capsule remains about the same size and is centered inside the transparent viewport with 6 px horizontal and vertical padding. Recording bars, ghost bars and transcribing dots were shifted by the same offset so the visual center remains aligned.
...
- rustfmt --check src-tauri/src/native_overlay.rs src-tauri/src/overlay.rs src-tauri/src/overlay_tests.rs

### Residual Risk

Real desktop visual confirmation under npm run tauri -- dev is still required because automated tests can verify dimensions and build behavior, but not actual Windows compositor rendering.
...
## 2026-06-02 V8 实时上屏语义确认和桌面浮窗底边裁切

### 现象

维护者手动测试百度实时 WebSocket API 后反馈效果可用，但需要确认目标输入框中的“实时上屏”是模拟流式输出还是真实流式输出。同时，底部胶囊状小浮窗整体视觉可接受，但胶囊底部边框看起来被截断。

### 根因

- 连续输入选择 `baidu-realtime` 时，前端调用 `start_baidu_realtime_session`，Rust worker 连接 `wss://vop.baidu.com/realtime_asr?sn=...`，发送 START 和 160 ms PCM frame，并把百度返回的 `MID_TEXT` / `FIN_TEXT` 通过 `voxtype-baidu-realtime-result` 事件发给前端。
- 前端只把 `payload.isFinal === true` 的 final 片段写入目标输入框；`MID_TEXT` 临时结果只更新主界面状态，不写入目标输入框。因此当前“实时上屏”不是本地模拟打字，也不是把 partial 临时文本反复改写，而是真实百度 WebSocket final 片段到达后分段上屏。
- 原生 Windows 浮窗和 WebView fallback 都使用 `120 x 32` 窗口，胶囊边框贴着窗口边界绘制。半透明边缘和抗锯齿在 layered window / WebView 透明窗口边缘处容易被裁切，视觉上表现为底部边框被切掉。

### 修复

- 保持实时 WebSocket 上屏逻辑不变：继续只上屏百度 `FIN_TEXT` final 片段，避免把不稳定 partial 直接写入目标输入框。
- 浮窗窗口高度从 `120 x 32` 调整为 `120 x 36`，视觉胶囊本体仍保持约 `120 x 32`，但在窗口内部向下/向上留出 2px 透明缓冲。
- Windows 原生浮窗将胶囊从 `y=2` 开始绘制，录音柱和转写点中心线同步从 `16` 调整到 `18`。
- WebView fallback 的 SVG viewBox 改为 `0 0 120 36`，胶囊 `y=2.5`，内容中心线同步到 `18`。

### 验证

- 新增前端测试覆盖 fallback SVG viewBox 和胶囊 y/height，防止再次贴边。
- 更新 Rust overlay 尺寸测试和 Tauri 配置断言为 `120 x 36`。
- 本机执行 Rust 测试二进制仍受既有 `STATUS_ENTRYPOINT_NOT_FOUND` 环境问题影响；需要使用 `cargo test --manifest-path src-tauri/Cargo.toml overlay --no-run`、`cargo check --manifest-path src-tauri/Cargo.toml --lib` 和维护者桌面手动观察补充验证。
...
## 2026-06-02 V9 postprocess insertion consistency

### Symptom

During V9 follow-up testing, the push-to-talk path could insert raw ASR text into the target input while the history layer saved post-processed text. Example: a rule such as `scale => skill` would appear in history but not necessarily in the target application.

### Impact

The visible text in the user's target app and the persisted recognition history could diverge. This weakened the V9 guarantee that deterministic glossary/replacement rules apply to final ASR text.

### Root Cause

`addTranscriptRecord` performed post-processing internally, but several final-text insertion paths called `insertTextWithClipboard` before that function. Clipboard insertion therefore used raw text while history normalization happened later.

### Fix

Post-process final ASR text before clipboard insertion for push-to-talk, continuous-input fallback, and Baidu realtime final events. `addTranscriptRecord` now accepts the already-computed postprocess result so history and insertion share the same normalized text and rule metadata.

### Verification

- `npm test -- --run src/App.test.tsx`: passed, 26 tests.
- `npm run typecheck`: passed.
- `npm run build`: passed.
- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml text_postprocess --no-run`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml transcript_history --no-run`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml audio_quality --no-run`: passed.
- `python -m json.tool docs/harness/feature_list.json`: passed.
- `git diff --check`: passed with only Git's existing CRLF/LF notice for `src/App.tsx`.

### Residual Risk

Real Tauri desktop manual verification is still required before marking `v9-001` passing: restart persistence, delete/clear persistence, glossary/replacement insertion in real target apps, and low-volume or far-microphone hint visibility.

## 2026-06-02 V9 realtime quality and text optimization UI follow-up

### Symptom

Maintainer reported three V9 regressions after desktop use: realtime continuous input did not show audio quality warnings, text optimization preview appeared ineffective, and the text optimization controls looked out of place inside the model settings page. The readiness area also had staggered rows, and long transcript history could be clipped at the bottom.

### Impact

Realtime Baidu WebSocket sessions could hide far-microphone or low-quality hints that short recordings already showed. Users could edit text optimization rules and preview without seeing the new rules applied, because preview used the backend's previously saved config. The model settings page mixed model routing with text cleanup controls, making the page harder to scan.

### Root Cause

- `finish_baidu_realtime_session` stopped the recorder but discarded `RecordedAudio.audio_quality`.
- The frontend realtime stop path created the transcript record without setting `lastAudioQualityRef` from the realtime summary.
- Text optimization preview called the backend preview command directly, which reads saved config, while the UI edits lived only in React state until save.
- The readiness panel used grid item placement for individual rows instead of two explicit columns, so CSS auto-placement produced offset rows.
- The history container mixed fixed/min heights and nested overflow rules, which could clip scrollable content.

### Fix

- Added `audioQuality` to the Baidu realtime session summary and carried it into transcript history records.
- Adjusted far-microphone detection so quiet continuous speech can report `possible_far_microphone`, not only mostly silent short clips.
- Split text optimization into a standalone `TextOptimizationView` with its own main-window entry and removed it from `ModelSettingsView`.
- Changed text optimization preview to save the current edited config before previewing, so the preview reflects the rules on screen.
- Rebuilt readiness status as two explicit columns and tightened transcript history scrolling.

### Verification

- `npm test -- --run src/App.test.tsx`: passed, 27 tests.
- `npm run typecheck`: passed.
- `npm run build`: passed.
- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml audio_quality --no-run`: passed.
- `rustfmt --check src-tauri/src/audio_quality.rs src-tauri/src/baidu_realtime.rs`: passed.
- `git diff --check`: passed with only Git's CRLF/LF notices.

### Residual Risk

Real Baidu realtime low-volume/far-microphone warning still needs maintainer desktop verification with actual microphone distance. Browser tests validate metadata plumbing and rendering, not acoustic behavior in the room.

## 2026-06-02 V9 audio warning false positives and mixed English diagnosis

### Symptom

Maintainer reported that almost every transcript row showed an audio quality warning, often `???`, even when recording with a headset microphone. Maintainer also reported that mixed Chinese sentences containing English words such as `skill` were not recognized, and it was unclear how to verify whether text optimization was working.

### Impact

Audio warnings became noisy enough to lose trust. Mixed English recognition could not be debugged from the main history alone because the UI only showed final text, not the raw ASR text before deterministic post-processing.

### Root Cause

The previous realtime warning follow-up made `possible_far_microphone` trigger when RMS was below `0.045` and active speech exceeded 240 ms. That made many normal continuous-speech recordings with conservative headset gain look like far-microphone recordings. For English words, the current Baidu Chinese `dev_pid` models and local Chinese whisper setting may fail before post-processing sees any usable approximation; deterministic rules can replace `scale => skill`, but cannot recover `skill` if ASR returns unrelated Chinese text or nothing close.

### Fix

- Made `possible_far_microphone` conservative again: it now requires low RMS, low peak, and a high silence ratio, instead of flagging quiet continuous speech by duration alone.
- Updated the audio quality regression test so quiet continuous speech alone does not produce `possible_far_microphone`.
- Added diagnostic log entries for final ASR paths that show `??` text, `????` text, and applied rule count. This gives a direct way to verify whether `skill` failed at ASR or at post-processing.
...
Mixed Chinese/English word recognition is still an ASR model/config quality issue when the raw ASR text does not contain a close English approximation. Next verification should record a real sentence containing `skill`, then inspect the diagnostic log's raw/optimized pair before changing model defaults or adding phrase-specific correction rules.


## 2026-06-03 V9 mostly_silence false positives and SenseVoice readiness mismatch

### Symptom

Maintainer reported that V9 `mostly_silence` warnings were too noisy, while low-volume, far-microphone, and clipping warnings were accurate. Maintainer also reported that SenseVoice Small can show ready in configuration, but a real recording test fails with a not-ready error and the insertion/status area turns red.

### Impact

The silence warning reduced trust in otherwise useful audio diagnostics. The SenseVoice readiness mismatch makes the model selection unsafe for real use because the UI can advertise readiness before the actual inference path is usable.

### Root Cause

`mostly_silence` only required a silence-frame ratio above 70%. A normal short utterance with leading or trailing quiet time can exceed that ratio even when it contains valid speech. The SenseVoice issue is intentionally not fixed in the V9 closeout; it is split to V9.5 because it involves model runtime readiness and real local inference, not V9 transcript quality/history diagnostics.

### Fix

- Tightened `mostly_silence`: it now requires silence ratio above 90% and active speech at or below 400 ms.
- Added a regression test proving a recording with one second of valid speech plus quiet tail does not report `mostly_silence`.
- Recorded SenseVoice as `v9.5-001` so readiness and inference can be debugged independently.

### Verification

- `npm test -- --run src/App.test.tsx`: passed, 28 tests.
- `npm run typecheck`: passed.
- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml audio_quality --no-run`: passed.
...
SenseVoice Small still needs focused V9.5 debugging: readiness must reflect actual runtime/model/token usability, and a real Tauri desktop recording must transcribe successfully before the feature can be marked passing.


## 2026-06-03 V9 audio warning thresholds still too strict

### Symptom

Maintainer reported that low-volume, far-microphone, clipping, and silence warnings still appeared too often in real desktop use. Some warnings appeared even when speaking volume and microphone distance felt normal.

### Impact

Audio quality hints became visual noise. If most transcript rows show warnings, users stop trusting the warning system, including the cases where it correctly detects real problems.

### Root Cause

The thresholds were still tuned like diagnostics, not product hints. low_volume depended only on RMS; possible_far_microphone allowed moderate silence ratios; clipping_risk fired before truly near-full-scale samples; mostly_silence still allowed too much active speech for a warning whose user meaning is nearly empty recording.

### Fix

- low_volume now requires both low RMS and low peak, reducing warnings for speech that has healthy peaks but low average energy.
- possible_far_microphone now requires very low RMS, low peak, high silence ratio, and at least a small amount of active speech, so it targets truly distant/weak speech rather than normal quiet speech.
- clipping_risk now requires a peak close to full scale.
- mostly_silence now requires a higher silence ratio and shorter active speech duration.
- Added regression tests for moderate low-level speech, sparse-but-present speech, and high-but-not-full-scale samples.

### Verification

- cargo test --manifest-path src-tauri/Cargo.toml audio_quality --no-run: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.
-pm test -- --run src/App.test.tsx: passed, 28 tests.
-pm run typecheck: passed.

### Residual Risk

These are still heuristic thresholds. The intended product behavior is conservative: warnings should appear only for clearly bad recordings. If real desktop use still shows noisy warnings, tune further toward fewer warnings rather than adding more visible warning categories.


## 2026-06-03 V9 mostly_silence still too sensitive

### Symptom

Maintainer reported that mostly_silence was still too sensitive after the broader audio-warning softening. Other warning categories are currently acceptable.

### Impact

A silence warning that appears during normal pauses or short utterances creates the same noisy-history problem as before. The product value is better if mostly_silence means the recording was essentially empty.

### Root Cause

The previous threshold still allowed up to 250 ms of active speech and 95% silent frames. That is still reachable with short valid utterances or recordings with normal leading/trailing quiet time.

### Fix

- Changed mostly_silence to require more than 98.5% silent frames.
- Reduced maximum active speech for the warning to 120 ms.
- Left low_volume, possible_far_microphone, and clipping_risk unchanged.
- Added regression coverage for both sides of the boundary.
...
This intentionally makes mostly_silence rare. That is acceptable for the current product direction: warnings should be high-confidence and sparse.


## 2026-06-03 V9 all audio warnings still too sensitive

### Symptom

Maintainer reported that every audio warning category was still too sensitive in real desktop use. The desired product behavior is sparse, high-confidence warnings rather than frequent diagnostic flags.

### Impact

Frequent warnings make the recognition history look broken and reduce trust. The warning system should only interrupt visual attention when the recording is clearly abnormal.

### Root Cause

The thresholds still reflected signal-analysis diagnostics more than user-facing product hints. Mild low average energy, normal quiet speech, non-full-scale loud samples, and short utterances with silence could still fall near warning boundaries.

### Fix

- Changed the product policy for V9 audio warnings to high-confidence only.
- low_volume: now requires extremely low RMS and low peak.
- possible_far_microphone: now requires very low RMS, low peak, more than 90% silence, and at least a small amount of active speech.
- clipping_risk: now requires near-full-scale peak.
- mostly_silence: now requires more than 99.5% silent frames and no more than 80 ms active speech.
- Updated regression tests to assert that moderate low-level speech, quiet continuous speech, high-but-not-full-scale samples, and sparse-but-present recordings do not warn.
...
This can miss mild audio issues. That is intentional for V9: warnings should be rare enough to remain meaningful. More detailed diagnostics can be exposed later in a diagnostics-only view if needed.


## 2026-06-03 continuous input does not start on second run

### Symptom

Maintainer reported that continuous input worked once, but a second attempt no longer inserted text.

### Impact

Daily continuous input becomes unreliable after one cycle. The user sees no useful result even though the shortcut can still be pressed.

### Root Cause

The non-realtime continuous-input stop path called stopRecording() and completed transcription, but did not reset isRecordingRef.current to false. The global shortcut listener only starts continuous input when toggleStartRecording arrives while !isRecordingRef.current; after the first cycle, the stale ref made later start events get ignored.

### Fix

- Set isRecordingRef.current = true after a successful startRecording() call.
- Set isRecordingRef.current = false after non-realtime continuous-input stopRecording() succeeds.
- Also reset the ref on non-realtime continuous-input stop failure.
- Extended the toggle shortcut regression test so it starts a second cycle after the first completed stop/transcribe/insert flow.

### Verification

- npm test -- --run src/App.test.tsx -t "runs the toggle shortcut closed loop": passed.
- npm test -- --run src/App.test.tsx: passed, 28 tests.
- npm run typecheck: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.
...
This fix covers the non-realtime continuous-input path. Baidu realtime already reset the ref in its stop success and failure paths, but real desktop retest should still verify both selected continuous-input models can run two cycles back to back.


## 2026-06-03 continuous input hotkey state mismatch

### Symptom

Maintainer reported that pressing the continuous-input shortcut several times did not record or insert text. After using push-to-talk once, continuous input started working again.

### Impact

Continuous input can appear dead without a clear error. Because push-to-talk still works, the underlying recorder and ASR stack are not necessarily broken; the failure is confusing and state-dependent.

### Root Cause

The toggle shortcut state exists in Rust, while the actual recording state is also tracked in the frontend. If a toggleStartRecording event is ignored or fails before the frontend starts recording, Rust still flips its internal toggle state. The next press emits toggleStopAndTranscribe while the frontend is idle. The previous frontend listener required toggleStopAndTranscribe && isRecordingRef.current, so it silently ignored this mismatch. Repeated presses could keep alternating between mismatched states.

### Fix

- Made the frontend listener self-heal toggle state mismatches.
- toggleStopAndTranscribe while frontend is idle is treated as a recovered start, with a diagnostic warning.
- toggleStartRecording while frontend is already recording is treated as a recovered stop, with a diagnostic warning.
- Added a regression test that sends toggleStopAndTranscribe while the frontend is idle and verifies recording starts instead of being ignored.

### Verification

- npm test -- --run src/App.test.tsx -t "recovers when toggle hotkey state": passed.
- npm test -- --run src/App.test.tsx: passed, 29 tests.
- npm run typecheck: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.

### Residual Risk

This is a frontend self-healing fix. A cleaner long-term design would expose an explicit Rust command to reset the toggle state after frontend start failures, but the current change prevents silent dead-key behavior and preserves user momentum.


## 2026-06-03 continuous input overlay appears but no text inserts

### Symptom

Maintainer reported that continuous input still failed: the desktop overlay animation appeared, but no text was inserted and no recognition record appeared. Push-to-talk worked, so the microphone/ASR path was not globally dead.

### Impact

The overlay made it look like continuous input had started, but the frontend recording/transcription path could be out of sync. This is worse than a clear failure because the user receives an active visual signal without a result.

### Root Cause

The earlier self-healing fix did not remove the underlying split-brain state. Rust owned a ToggleDictationState that alternated start/stop actions, while the frontend also owned the actual recording state. Rust can show the overlay before the frontend confirms recording. If frontend start fails, is delayed, or is ignored, Rust still advances its toggle state. Later presses can emit the opposite action from what the frontend actually needs.

### Fix

- Removed stateful start/stop authority from Rust ToggleDictationState.
- Ctrl+Alt+V press now emits a stateless toggleStartRecording action, treated as "toggle pressed" for compatibility with existing payload names.
- The frontend is the single authority for continuous-input start/stop because it knows the actual recording state.
- Frontend tests now model repeated toggleStartRecording events as start then stop.
- Rust hotkey tests now assert toggle press is a frontend-owned toggle signal.

### Verification

- cargo test --manifest-path src-tauri/Cargo.toml hotkey --no-run: passed.
- npm test -- --run src/App.test.tsx -t "toggle": passed, 4 tests.
- npm test -- --run src/App.test.tsx: passed, 29 tests.
- npm run typecheck: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.
- python -m json.tool docs/harness/feature_list.json: passed.
...
Real desktop retest is still required because this bug depends on OS hotkey timing and Tauri event delivery. The key invariant is now simpler: Rust fires the hotkey event and overlay hint; the frontend alone decides whether a Ctrl+Alt+V press starts or stops actual recording.


## 2026-06-03 continuous input intermittent after frontend-owned toggle

### Symptom

Maintainer reported that after today's changes continuous input was only intermittently usable. Sometimes it worked; other times the overlay appeared but there was no text insertion and no recognition history row.

### Impact

This is a regression from today's toggle-state changes. It makes the continuous-input shortcut unreliable even though the global shortcut and overlay path are active.

### Root Cause

Moving Ctrl+Alt+V toggle authority from Rust to the frontend removed one split-brain state, but introduced a frontend race: startRecording and stop/transcribe are asynchronous. While a start or stop is in flight, duplicate pressed events can arrive before isRecordingRef and React state settle. Without an in-flight guard, the frontend can process a second toggle against stale state.

### Fix

- Added toggleActionInFlightRef to serialize continuous-input toggle actions.
- Duplicate toggleStartRecording or legacy toggleStopAndTranscribe events are ignored while a previous continuous-input start/stop action is still running.
- The guard is cleared in a finally callback after the action settles.
- Added a regression test that keeps startRecording unresolved, sends duplicate toggle events, and verifies only one start occurs; once start settles, the next toggle can stop.

### Verification

- npm test -- --run src/App.test.tsx -t "ignores duplicate continuous-input": passed.
- npm test -- --run src/App.test.tsx: passed, 30 tests.
- npm run typecheck: passed.
- cargo test --manifest-path src-tauri/Cargo.toml hotkey --no-run: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.
- python -m json.tool docs/harness/feature_list.json: passed.
...
Real desktop retest is still needed because OS global-shortcut delivery can produce timing patterns unit tests only approximate. The new invariant is that only one continuous-input toggle action can run at a time.


## 2026-06-03 continuous input overlay appeared without insertion

### Symptom

Maintainer reported that Ctrl+Alt+V continuous input still sometimes worked and sometimes failed. In failing cases the desktop overlay appeared, but no text was inserted and no recognition history row was created.

### Impact

The visible overlay became a false-positive signal. It told the user continuous input was active even if the frontend had not successfully entered the recording/transcription path.

### Root Cause

Even after moving toggle authority to the frontend, Rust still showed the overlay immediately for toggle hotkey actions. This preserved a split between OS hotkey receipt and actual frontend recording state. If the frontend event was delayed, ignored, or protected by the in-flight guard, the overlay could still appear without a real recording session behind it.

### Fix

- Rust no longer calls show_dictation_overlay or show_transcribing_overlay for Ctrl+Alt+V toggle actions.
- Rust still handles push-to-talk overlay pre-display because press/release semantics are owned by Rust there.
- Frontend continuous input now calls showDictationOverlay only after startRecording or startBaiduRealtimeSession succeeds.
- Tests assert frontend-owned overlay display for regular continuous input and Baidu realtime starts.

### Verification

- npm test -- --run src/App.test.tsx -t "toggle|realtime": passed, 11 tests.
- npm test -- --run src/App.test.tsx: passed, 30 tests.
- npm run typecheck: passed.
- cargo check --manifest-path src-tauri/Cargo.toml --lib: passed.
- cargo test --manifest-path src-tauri/Cargo.toml hotkey --no-run: passed.
- python -m json.tool docs/harness/feature_list.json: passed.
- git diff --check: passed with only CRLF/LF notices.

### Residual Risk

Real desktop retest is required. The expected behavior after this fix is: for Ctrl+Alt+V, no recording overlay should appear unless the frontend has actually started the continuous-input recording/session.


## 2026-06-03 SenseVoice ready in UI but shortcut path says not ready

### Symptom

Maintainer reported that SenseVoice Small could show ready in the UI/configuration path, but a recording test could still fail as not ready. The main insertion/status surface turned red, creating a false readiness signal.

### Impact

This blocks V9.5 because the user cannot trust the model readiness indicator. A local model that appears ready must either start recording and attempt inference, or show a specific actionable configuration/runtime error.

### Root Cause

The frontend global-shortcut listener is registered once. It uses closure-captured values unless a mutable ref is used for state that changes after startup. ASR and cloud readiness had ref-backed shortcut-time checks, but SenseVoice readiness was still read from the render closure. Therefore after one-click install or config refresh changed SenseVoice to ready, the visible UI updated while Ctrl+Alt+V could still evaluate the original not-ready SenseVoice status.

### Fix

- Added `senseVoiceConfigStatusRef` beside the existing ASR/cloud readiness refs.
- Updated `applySenseVoiceConfigStatus` to refresh the ref immediately when config status is loaded, saved, or installed.
- Changed shortcut-time `getModelReadiness` calls for push-to-talk and continuous input to use the latest SenseVoice ref.
- Added a regression test for install-to-ready -> select `sensevoice-small` for continuous input -> trigger global shortcut -> recording starts.
- Replaced unreadable question-mark diagnostic titles in the touched continuous-input overlay path with a stable English title.

### Verification

- `npm test -- --run src/App.test.tsx -t "refreshed SenseVoice readiness"`: passed.
- `npm test -- --run src/App.test.tsx`: passed, 31 tests.
- `npm run typecheck`: passed.
- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml sensevoice --no-run`: passed.
- `python -m json.tool docs/harness/feature_list.json`: passed.
- `bash init.sh`: passed.

### Residual Risk

This fixes a verified stale-frontend-readiness failure. It does not by itself prove the installed sherpa-onnx runtime, ONNX model, and tokens can complete real local inference on the maintainer machine. V9.5 remains in progress until a real Tauri desktop SenseVoice recording successfully transcribes and inserts text, or any runtime/inference failure is captured with a specific error.

## 2026-06-03 SenseVoice CLI rejects separated option arguments

### Symptom

Maintainer retested SenseVoice Small after reinstall. The app used the new CLI runtime but failed with: `Invalid option --sense-voice-model (option format is --x=y)`.

### Root Cause

The previous GUI-runtime issue was fixed by moving to `sherpa-onnx-offline.exe`, but the invocation still used separated CLI arguments such as `--sense-voice-model <path>`. The installed sherpa-onnx CLI parser requires equals-form options: `--sense-voice-model=<path>`, `--tokens=<path>`, `--sense-voice-language=<lang>`, and `--debug=0`.

### Fix

- Updated SenseVoice argument construction to use sherpa-onnx equals-form options.
- Kept the Windows no-window process creation path for the CLI runtime.
- Confirmed the managed config points at `sherpa-onnx-offline.exe`.
- Confirmed the deprecated single-file GUI runtime is absent from the managed SenseVoice bin directory.

### Verification

- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml sensevoice --no-run`: passed.
- `sherpa-onnx-offline.exe --help | grep sense-voice`: confirmed `--sense-voice-model` and `--sense-voice-language` options exist and CLI help examples use `--x=value` style.
- Running the Rust test binary still fails in this Windows environment with the existing `STATUS_ENTRYPOINT_NOT_FOUND` startup issue; the test binary builds successfully.

### Residual Risk

Real Tauri desktop SenseVoice recording still needs maintainer retest. The expected failure mode should no longer be `Invalid option --sense-voice-model`; if inference still fails, the error should include stdout/stderr summary from the CLI.

## 2026-06-03 SenseVoice JSON output inserted verbatim and auto language misdetected

### Symptom

Maintainer retested SenseVoice Small after the CLI option-format fix. The runtime no longer failed, but the inserted text was a full JSON object such as `{"lang":"<|ko|>", ... "text":"?"}`, not the user speech.

### Root Cause

The sherpa-onnx SenseVoice CLI prints structured JSON lines. VoxType's parser treated the last non-empty stdout line as plain text, so it inserted the entire JSON payload. In addition, managed SenseVoice config defaulted to `auto`, which misdetected a Chinese test as Korean in this case.

### Fix

- Updated sherpa stdout parsing to detect JSON and return only the `text` field.
- Changed SenseVoice default/managed language from `auto` to `zh` for VoxType's Chinese-first product default.
- Updated the maintainer machine's existing SenseVoice config from `auto` to `zh`.

### Verification

- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml sensevoice --no-run`: passed earlier after the argument-format fix and remains structurally covered by the new parser test source.
- Confirmed current app config now has `"language": "zh"`.

### Residual Risk

Real Tauri desktop SenseVoice retest is still required. Expected output is now only the recognized text field, not a JSON object. Recognition content may still need evaluation after forcing Chinese language.

## 2026-06-04 V10 audio enhancement enabled but no visible metadata

### Symptom

Maintainer enabled audio enhancement and tested near/far recordings, but did not see any transcript metadata such as 增强 or 裁剪. This made the feature impossible to verify from the UI: it was unclear whether preprocessing ran, whether it made no change, or whether metadata was dropped.

### Impact

V10 audio enhancement looked like a non-observable switch. Without visible metadata, the maintainer could not validate the feature in real Tauri desktop use, and normal recordings could be misread as enhancement doing nothing even when the pipeline executed safely.

### Root Cause

The backend returned transcript.audioPreprocess, and the main window already knew how to render record.audioPreprocess, but the frontend did not persist the override into PersistedTranscriptEntry. Some stop-then-transcribe paths also failed to pass transcript.audioPreprocess into addTranscriptRecord.

There was a second product-level observability issue: when enhancement was enabled but no material trim/gain was needed, the pipeline could report no applied change, so the history row showed nothing even though the preprocessing path had run.

### Fix

- Persisted audioPreprocessOverride into transcript history entries.
- Passed transcript.audioPreprocess through push-to-talk and continuous stop-then-transcribe fallback paths.
- Changed enabled preprocessing semantics so an enabled pipeline reports that it ran, even when the recording needed no material change.
- Rendered the no-material-change case as 增强 / 无明显变化.
- Moved audio enhancement into its own 设置 -> 音频增强 tab so it is not confused with model provider configuration.
- Moved text optimization into 设置 -> 文本优化 and removed the separate top-level main-window button.

### Verification

- npm test -- --run src/App.test.tsx: passed, 35 tests.
- npm run typecheck: passed.

### Residual Risk

Real Tauri desktop verification is still required. Expected behavior after this fix: enable 设置 -> 音频增强, record once, and the new history row should show 增强 / 裁剪 Nms, 增强 / Nx, or 增强 / 无明显变化. Disabling enhancement should stop adding enhancement metadata to new records.

## 2026-06-04 SenseVoice returns empty text after V10 enhancement

### Symptom

Maintainer enabled V10 audio enhancement and tested SenseVoice Small. sherpa-onnx completed normally and printed JSON with lang `<|zh|>`, event `<|Speech|>`, but `text` was empty. VoxType surfaced this as `SenseVoice Small returned no text` and the transcription failed.

### Impact

The model/runtime path was no longer the problem: recognizer creation, WAV input, and process execution all succeeded. The user-facing failure came from passing the enhanced stop-then-transcribe samples directly into SenseVoice and treating an empty enhanced result as final.

### Root Cause

V10 preprocessing was applied before model routing for all stop-then-transcribe models, including SenseVoice Small. SenseVoice is more sensitive to the current conservative normalization/VAD-trim output than Baidu Short Speech or whisper.cpp. When sherpa-onnx returned an empty JSON `text`, VoxType had no model-specific fallback to retry the original raw samples.

### Fix

- Added a SenseVoice-specific fallback around the V10 preprocessing path.
- If preprocessing is disabled, SenseVoice behavior is unchanged.
- If preprocessing is enabled and SenseVoice succeeds on processed samples, the transcript keeps audio preprocessing metadata.
- If preprocessing is enabled but SenseVoice returns the known empty-text error, VoxType retries SenseVoice once with the original raw samples.
- If the raw retry succeeds, no enhancement metadata is attached, because the accepted transcript came from raw audio.
- Other SenseVoice errors are not swallowed or retried as raw audio.

### Verification

- Red test first: `cargo test --manifest-path src-tauri/Cargo.toml sensevoice_empty_preprocessed_result_retries_raw_audio --no-run` failed because `transcribe_sensevoice_with_preprocess_fallback` did not exist.
- After implementation: `cargo test --manifest-path src-tauri/Cargo.toml sensevoice_empty_preprocessed_result_retries_raw_audio --no-run` passed.

### Residual Risk

Real Tauri desktop retest is required. Expected behavior: with enhancement enabled and SenseVoice selected, a processed-audio empty result should automatically retry raw audio. If raw audio is recognized, text should insert normally; if raw audio is also empty, the same no-text error can still surface.

### Follow-up: fallback was invisible in history

Maintainer confirmed recognition became normal, but no enhancement information appeared. The previous fallback deliberately omitted metadata when raw audio won, which hid the fact that enhancement had been tried and then bypassed.

Fix update: `AudioPreprocessSummary` now carries `fallback_to_raw`. Frontend history renders that case as `增强 / 已回退`. This keeps the UI honest: the enhancement pipeline was attempted, but the accepted transcript used raw audio.

Additional verification: `npm test -- --run src/App.test.tsx -t fallback` passed, `npm test -- --run src/App.test.tsx` passed with 36 tests, `npm run typecheck` passed, `cargo check --manifest-path src-tauri/Cargo.toml --lib` passed, and `cargo test --manifest-path src-tauri/Cargo.toml sensevoice_empty_preprocessed_result_retries_raw_audio --no-run` passed.

## 2026-06-04 V10 enhancement settings reset and metadata disappeared

### Symptom

Maintainer reported that audio enhancement returned to off after every restart, and completed recordings still showed no enhancement metadata or fallback metadata.

### Root Cause

Two persistence boundaries were broken. First, the frontend imported `getAudioPreprocessConfig` but never called it during startup, so the Settings UI always started from the default-disabled local state. Second, Rust transcript history did not include `audio_preprocess` in `PersistedTranscriptEntry`; after the frontend saved a new record, Rust returned history entries with that field dropped, and the frontend replaced its local list with the stripped saved list.

### Fix

- App startup now calls `getAudioPreprocessConfig` and applies the returned config to the Settings UI.
- `transcript_history::PersistedTranscriptEntry` now includes `audio_preprocess: Option<AudioPreprocessSummary>`.
- Added a frontend regression test for loading persisted enabled enhancement config on startup.
- Added a Rust regression test that saves and reloads `audio_preprocess` metadata including `fallback_to_raw`.

### Verification

- `npm test -- --run src/App.test.tsx -t persisted`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml saves_and_loads_audio_preprocess_metadata --no-run`: passed.
- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.

### Residual Risk

Real Tauri desktop retest is required. Expected behavior: enabling audio enhancement remains enabled after app restart, and new recognized records keep enhancement metadata after persistence refresh.


## 2026-06-05 Dolby Voice popup and SenseVoice empty text report

### Symptom

Maintainer reported that pressing the dictation shortcut triggers a Dolby Voice noise suppression activation popup. The same test also blocked insertion-strategy validation because SenseVoice Small completed sherpa-onnx execution but returned JSON with `text` as an empty string, surfacing `SenseVoice Small returned no text`.

### Current Assessment

This issue is not fixed in this documentation-only turn. Local code inspection indicates VoxType does not directly trigger a Dolby-specific popup; pressing the shortcut starts microphone capture, and the popup is likely produced by the system audio enhancement or device driver stack. The SenseVoice failure occurs before insertion strategy execution, so it blocks testing SendInput/clipboard strategy but is not itself an insertion-strategy failure.

### Verification Status

Low-risk automated checks were run after documenting the testing strategy. They do not validate the real microphone, Dolby popup, global shortcut, or real SenseVoice desktop recording path. Manual desktop validation remains required.

### Next Debugging Direction

Export or inspect the last ASR input WAV after a failing SenseVoice run, compare audio quality metrics, and determine whether system noise suppression or the recorded sample itself causes SenseVoice to emit an empty text field. Consider adding a future Safe Desktop Regression Harness sentinel that can feed a fixed WAV through the SenseVoice path without using the real microphone.


## 2026-06-05 SendInput returned 0 inputs

### Symptom

Maintainer reported `语音识别失败：文本上屏失败：SendInput failed: 0/16（insertion_failed）` after a successful recognition path attempted text insertion.

### Impact

The ASR result was blocked at the insertion layer. Because the selected strategy was pure `SendInput`, VoxType surfaced the failure directly instead of falling back to clipboard. The `Auto` strategy already falls back to clipboard, but explicit `SendInput` does not.

### Root Cause

The Windows FFI definition of `INPUT` was too small. The union only contained `KEYBDINPUT`, so `std::mem::size_of::<Input>()` did not match the Win32 `INPUT` size expected by `SendInput` on x64. Passing the wrong `cbSize` can make `SendInput` reject the whole batch and return 0, which produced `SendInput failed: 0/16`.

### Fix

Added a Windows `MouseInput` struct and included it in the `InputUnion`, so the Rust `Input` layout matches the Win32 `INPUT` union size. Added platform-gated regression tests asserting `Input` is 40 bytes on x64 Windows and 28 bytes on x86 Windows.

### Verification

- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml insertion --no-run`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml insertion_result --no-run`: passed.
- `python -m json.tool docs/harness/feature_list.json`: passed.
- Direct Rust test execution still fails with the existing Windows `STATUS_ENTRYPOINT_NOT_FOUND` test-binary startup issue; the new regression test was compiled but not executed in this environment.

### Residual Risk

Real desktop validation is still required. Expected retest: select `SendInput`, focus Notepad or VS Code, and run a short dictation. If `SendInput` still fails in a specific target, switch to `Auto` to confirm clipboard fallback and record the target in the V12 compatibility matrix.


## 2026-06-05 Auto insertion repeated Chinese tail in Notepad

### Symptom

Maintainer tested continuous input with insertion strategy `Auto`. Transcript history stored the intended Chinese text, but Notepad showed the beginning correctly and then repeated the final Chinese character many times. The history metadata showed `上屏 auto -> sendinput`.

The same record showed model metadata as `百度实时 WebSocket API` even though the maintainer expected SenseVoice.

### Impact

The recognition result and history persistence were correct, but direct `SendInput` was unsafe for this Chinese target path. Because `Auto` considered the Win32 API call successful, it did not fall back to clipboard. This produced visibly corrupted target text while metadata reported success.

### Root Cause

`Auto` previously tried `SendInput` first for all text. For Chinese/non-ASCII text, Windows Unicode `SendInput` can interact badly with target controls or IME/composition behavior: the API may return success while the target receives repeated or malformed text. There is no reliable post-insertion verification in the current architecture, so API success is not enough evidence that text was inserted correctly.

The model label issue is probably a routing/expectation mismatch: a transcript record labeled `百度实时 WebSocket API` means the continuous-input route actually used `baidu-realtime`. Opening or configuring SenseVoice does not by itself make the continuous-input default model SenseVoice; Settings -> Model must set the `连续输入` default model to SenseVoice. This still needs maintainer retest and may need clearer UI copy.

### Fix

Changed `Auto` insertion to prefer clipboard for any non-ASCII text. ASCII text can still try `SendInput` first, but Chinese text now routes as `auto -> clipboard` with error/category metadata `non_ascii_text`. This makes Auto conservative for the main Chinese-first use case and avoids sending Chinese text through the risky SendInput path.

Also added a frontend diagnostic label for `non_ascii_text` as `Chinese text uses clipboard`.

### Verification

- `cargo check --manifest-path src-tauri/Cargo.toml --lib`: passed.
- `cargo test --manifest-path src-tauri/Cargo.toml insertion --no-run`: passed.
- `npm test -- --run src/App.test.tsx -t insertion`: passed, 2 tests.
- `npm run typecheck`: passed.

### Residual Risk

Manual desktop validation is required. Expected retest: choose `Auto`, dictate Chinese into Notepad, and confirm metadata shows `上屏 auto -> clipboard` instead of `auto -> sendinput`; Notepad should match the history text. Separately confirm Settings -> Model has `连续输入` set to the intended model before testing SenseVoice continuous input.


## 2026-06-05 SenseVoice history model label shown as Baidu Realtime

### Symptom

Maintainer configured both push-to-talk and continuous input to use SenseVoice Small, but transcript history metadata still displayed `百度实时 WebSocket API`.

### Investigation

The local user preferences file confirmed the actual saved routing was correct: `toggleDictationModel` was `sensevoice-small` and insertion strategy was `auto`. Therefore the issue was not model preference persistence or continuous-input routing.

### Root Cause

`MainWindow.tsx` formatted transcript record model metadata with a partial mapping. It handled `local-whisper` and `baidu-short`, then used `百度实时 WebSocket API` as the fallback label for every other model. As a result, records with `modelId: sensevoice-small` were rendered as Baidu Realtime even though the underlying record model was SenseVoice.

### Fix

Added an explicit `sensevoice-small -> SenseVoice Small` branch in `formatModelLabel`, and added a regression test that persisted SenseVoice history records render `SenseVoice Small` and do not render `百度实时 WebSocket API`.

### Verification

- `npm test -- --run src/App.test.tsx -t SenseVoice`: passed, 3 tests.
- `npm run typecheck`: passed.

### Residual Risk

This fixes the displayed model metadata. It does not by itself prove real SenseVoice recognition quality or the real desktop insertion path; those still require desktop retest.

## 2026-06-05 SendInput returns success but repeats the final character

### Symptom

After the initial `SendInput failed: 0/16` layout fix, maintainer tested English input. Recognition history showed the correct transcript `test i have a headache`, but the target editor showed repeated final letters such as `test eeeeeeeeeeeeeeeee`. A previous Chinese test had also repeated the final Chinese character while history text was correct.

### Impact

This is an insertion-layer corruption bug after ASR has already produced correct text. It affects `SendInput` and `Auto` when `Auto` chooses `SendInput`; clipboard insertion remains the reliable baseline.

### Root Cause

The hard failure root cause was the wrong Win32 `INPUT` union size and was already fixed. The repeated-character follow-up exposed a second issue: VoxType submitted all UTF-16 down/up events for a transcript in one large `SendInput` batch and treated API success as text-insertion success. At least one real target control can accept the batch but process it as repeated final input. Because the history text is correct, the corruption is not ASR, model routing, or transcript persistence.

### Fix

`src-tauri/src/insertion/mod.rs` now constructs one Unicode down/up pair per UTF-16 code unit and calls `SendInput(2, ...)` per pair, with a 1 ms gap between pairs. The full x64/x86 `INPUT` layout guard remains. A Windows-gated regression test checks that the generated pair preserves `wVk = 0`, the expected `wScan`, `KEYEVENTF_UNICODE` on key down, and `KEYEVENTF_UNICODE | KEYEVENTF_KEYUP` on key up.

### Verification

Automated checks only, no real desktop automation:

- `cargo check --manifest-path src-tauri/Cargo.toml --lib`
- `cargo test --manifest-path src-tauri/Cargo.toml insertion --no-run`
- `npm test -- --run src/App.test.tsx -t SenseVoice`
- `npm test -- --run src/App.test.tsx -t insertion`
- `npm test -- --run`
- `npm run typecheck`
- `rustfmt src-tauri/src/insertion/mod.rs --edition 2021`

### Residual Risk

The true pass/fail signal is still real desktop target behavior. Retest `SendInput` and `Auto` in Notepad, VS Code, and browser text fields. If any target still repeats characters, `Auto` should be demoted to clipboard for all text until a deeper Windows insertion strategy is chosen.


## 2026-06-05 SendInput Unicode hypothesis disproved by retest

### Symptom

Maintainer retested `SendInput` and `Auto` with English `test i have a headache` after the per-code-unit Unicode change. Notepad still showed `test eeeeeeeeeeeeeeeee` while recognition history was correct.

### Updated Root Cause

The previous hypothesis that a whole-transcript batch caused the repeated final character was wrong. Sending each UTF-16 code unit separately through `KEYEVENTF_UNICODE` still reproduced the failure.

Public documentation and community reports point to the lower-level issue: `KEYEVENTF_UNICODE` uses the `VK_PACKET` / `WM_CHAR` path rather than normal virtual-key events. Win11 Notepad and some target controls are known to behave poorly with automation text paths. For ASCII text, VoxType should not use Unicode packet injection.

### Fix

`src-tauri/src/insertion/mod.rs` now maps ASCII characters through `VkKeyScanW` and sends ordinary virtual-key down/up events. `Auto` continues to route non-ASCII text to clipboard. Explicit `SendInput` for non-ASCII now fails clearly instead of pretending success.
...
This still requires real desktop validation. The next maintainer test should use Notepad plus VS Code/browser fields. If Notepad still repeats characters, direct insertion should be classified as incompatible with that target and `Auto` should become clipboard-only until a TSF or target-specific path is designed.
...
After switching ASCII SendInput from Unicode packet injection to `VkKeyScanW` virtual-key events, maintainer reported that Notepad no longer repeated a single character, but the target English text still did not exactly match the accurate history transcript.

### Updated Hypothesis

The input path is triggered by global shortcuts. Even when Rust receives a release event later, insertion can begin while Ctrl/Alt/Shift/Win are still physically down or logically active in the target thread. Normal virtual-key events then become modified keystrokes, producing target text that differs from the transcript.

### Fix

Before typing via SendInput, VoxType now waits up to 400 ms for Shift/Ctrl/Alt/Win to be physically released using `GetAsyncKeyState`, then sends synthetic key-up events for Shift, Ctrl, Alt, left Win, and right Win. Only after that does it send ASCII virtual-key down/up pairs.
...
This is intentionally the last direct SendInput stabilization attempt. If the next Notepad/VS Code/browser retest still diverges from the history text, `Auto` should become clipboard-only and `SendInput` should remain an explicit experimental strategy.
