...
## SenseVoice Small / sherpa-onnx local runtime research

### ?? 2026-06-02

?????????????? / FunAudioLLM ??? SenseVoice Small????????????? Python ????
...
- ModelScope SenseVoice demo/model reference: https://www.modelscope.cn/studios/iic/SenseVoice
- ??? release repo: https://github.com/shandianshuo/shandianshuo-releases
- ?????: https://shandianshuo.cn
- ????????????????? `sensevoice-small`????? 1G?????????? pin / X ??????????????????
- sherpa-onnx official docs: https://k2-fsa.github.io/sherpa/onnx/index.html
- sherpa-onnx SenseVoice docs: https://k2-fsa.github.io/sherpa/onnx/sense-voice/index.html
- sherpa-onnx SenseVoice pretrained models: https://k2-fsa.github.io/sherpa/onnx/sense-voice/pretrained.html
- k2-fsa/sherpa-onnx releases: https://github.com/k2-fsa/sherpa-onnx/releases
- SenseVoice ONNX model repo: https://huggingface.co/csukuangfj/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17

???

- ???? FunASR / ModelScope ? Python API ?????? `iic/SenseVoiceSmall`??????????? Python ????????????????
- ??????????????????????????? Python???????????????????????????????????
- `sherpa-onnx` ???????????? ASR?? ONNX Runtime ?? PyTorch???? Windows ?????????? SenseVoice ???????? SenseVoice ONNX ???
- VoxType ???? `sherpa-onnx` ??????? SenseVoice Small ?????????? Windows `sherpa-onnx-non-streaming-asr-x64`?`model.int8.onnx` ? `tokens.txt`?????????????? tokens ???

?????

- ????????????? Tauri ??????? SenseVoice ???
- `sherpa-onnx` CLI ??????????? `--help` ?????????????????? CLI ????????? `SenseVoiceSmallEngine` ??????
- ????? runtime ?? URL ??????????????????
