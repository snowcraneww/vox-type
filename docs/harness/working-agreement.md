...
- Git 提交必须使用项目级身份 `VoxType <maintainers@voxtype.dev>`。
- 不要把个人姓名、公司邮箱、本机用户名、用户目录、环境变量值或 API key 写入提交元数据、文档、日志或测试快照。
- 提交前如果改过 Git 配置或执行过历史重写，必须重新扫描 commit metadata 和仓库内容。
