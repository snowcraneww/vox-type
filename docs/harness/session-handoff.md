...
- UI 方向：主界面克制专业，不再使用夸张仿苹果窗口装饰；语音状态由小型彩色流光波纹浮窗表达。
- 重要边界：TSF、完整模型市场、云端 ASR 和 AI 改写不在 V2 默认范围内。

## 仍可优化或待增强

- 识别质量仍可优化：输入设备选择已接入但未持久化；模型选择、音量提示、VAD 和降噪都还没有做。
- 文本输出标准化仍未做：另一台机器 whisper.cpp 返回繁体中文，下一阶段需要增加简体中文优先策略，例如 whisper prompt、后处理或 OpenCC 类转换。
...
- 当前下一任务：`v2-001` 第二版日常语音输入体验。
- 为什么它是下一步：第一版 MVP 已经跑通，下一阶段应降低日常使用摩擦并让输出更稳定。
- 建议优先级：先实现克制主界面和彩色流光语音浮窗，再接入简体中文输出偏好、输入设备选择持久化和全局快捷键按住说话。
