...
- Modify: `src/ModelSettingsView.tsx` or a dedicated settings panel to expose audio enhancement controls.
- Modify: `src/App.tsx` to render preprocessing metadata in transcript rows when present.
...
- [ ] Add settings UI with a master toggle and conservative preset controls.
- [ ] Render compact transcript metadata when preprocessing was applied.
- [ ] Add tests for loading settings, saving settings, and rendering preprocessing metadata.
