...
Add an audio enhancement section after V9 diagnostics exist:

- master toggle: `Audio enhancement`,
- conservative preset: normalization + silence trim,
- advanced toggles hidden or secondary: high-pass, denoise spike result,
- show before/after diagnostics: RMS, peak, silence ratio, warnings cleared or introduced,
- provide an export command for diagnostic WAVs when troubleshooting is explicitly requested.

Do not expose raw threshold sliders in the first UI unless tests show the defaults are not robust.
