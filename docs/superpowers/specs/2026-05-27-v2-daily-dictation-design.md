...
状态：维护者已批准方向，第二版主体实现已接入代码，仍需维护者手动验证全局快捷键真实闭环。本文是 V2 的产品、交互和技术设计，实施计划见 `docs/superpowers/plans/2026-05-27-v2-daily-dictation.md`。
...
2. 用户按住 VoxType 快捷键。
3. 主窗口里的语音状态面板显示彩色流光波纹；后续再拆成独立置顶小浮窗。
4. 用户说话时，录音态波纹持续起伏；后续再绑定实时音量。
5. 用户松开快捷键。
6. 浮窗进入转写状态。
...
第一版实现使用 CSS 频谱柱和流动光带，不使用 WebGL/Three.js。视觉不需要追求复杂 3D，关键是状态表达准确、运动顺滑、颜色有层次。
...
|---|---|---|
| `idle` | 已就绪 | 慢速柔光短波纹 |
| `recording` | 正在听 | 彩色频谱柱错峰起伏；后续接入真实音量 |
...
- 前端事件监听：接收 Rust 发出的 `voxtype-push-to-talk`，复用现有录音、转写和上屏流程。
- overlay window 管理：当前先在主窗口内做状态面板预览；后续再创建小型置顶窗口。
- 简体中文优先：当前通过 whisper.cpp prompt 实现第一步；如果仍返回繁体，再增加后处理。
- 配置扩展：当前已保存输入设备；快捷键和输出文字偏好后续再做完整设置页。
...
sequenceDiagram
  ...
  participant Rust as Rust 快捷键事件
  ...
  User->>Hotkey: 按住快捷键
  Hotkey->>Rust: press
  Rust->>UI: startRecording event
  UI->>Recorder: start_recording
  User->>Hotkey: 松开快捷键
  Hotkey->>Rust: release
  Rust->>UI: stopAndTranscribe event
  UI->>Recorder: stop_recording
  UI->>ASR: transcribe
  ASR-->>UI: transcript
  UI->>Target: clipboard paste
  UI->>UI: succeeded or failed
```

## 风险

- Tauri overlay window 尚未拆出；后续在 Windows 上的置顶、透明、焦点行为需要手动验证。
...
- 用户能看到彩色流光语音状态面板，并能区分待命、录音、转写、成功、失败状态。
