...
状态：草案，等待维护者确认。确认后应转入 `openspec/changes/<change-id>/proposal.md`，再拆成实现计划。本文不是实现任务清单，不代表已经批准 scaffold 产品代码。
...
- 上屏策略分层。先剪贴板，再 `SendInput`，最后 TSF。
...
1. 第一版语言目标：中文优先，还是中英双语。
2. 第一版是否接受剪贴板上屏方案。
3. 第一版 ASR 是否优先试 whisper.cpp 路线，还是 sherpa-onnx 路线。
4. 第一版 UI 是否接受“托盘 + 设置页 + 状态提示”的工具形态。

我的默认建议：中文优先兼容英文；接受剪贴板上屏作为 MVP；ASR adapter 同时保留 whisper.cpp 和 sherpa-onnx 两条路，但第一轮只实现一个；UI 先做托盘和设置页。
