...
- 按住快捷键真实录音。
- 松开快捷键自动转写。
- 自动下载或管理 whisper.cpp 模型。
- 像正式输入法一样在所有软件中稳定上屏。
- TSF 输入法框架集成。

当前最准确的描述是：项目已经有可运行桌面壳、UI、状态模型、麦克风探测、真实录音采集、16 kHz ASR 输入准备、whisper.cpp 转写命令和剪贴板上屏 adapter；但“真实转写后自动上屏”的完整闭环还在实现中。
...
- 如果提示缺少 `VOXTYPE_WHISPER_CPP_BINARY` 或 `VOXTYPE_WHISPER_CPP_MODEL`，说明环境变量没设置。
...
| asr | 有 mock、whisper.cpp CLI adapter 和 `转写最近录音` command | 配置真实 binary/model，并手动验证识别质量 |
...
2. 手动验证“录音 -> 转写最近录音”能返回文本。
3. 把真实转写文本接到剪贴板上屏，形成一键 proof-of-life。
