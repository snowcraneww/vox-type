...
- 按 `Ctrl+Alt+V` 开始录音，再按一次停止。这个模式现在会尝试实验分段实时输入：录音过程中每隔几秒转写新增片段，并把文本粘贴到当前光标位置；如果录音过程中没有任何片段成功上屏，停止时会用整段录音兜底转写并上屏。
...
当前最准确的描述是：项目已经有可运行桌面壳、深色 V3 主界面、状态模型、彩色语音波纹、麦克风探测、输入设备列表、输入设备持久化、全局按住说话事件、全局切换录音事件、真实录音采集、16 kHz ASR 输入准备、whisper.cpp 一键安装/转写命令、实验分段实时输入、诊断 WAV 导出和剪贴板上屏 adapter；后续重点是识别质量、模型选择、快捷键配置、真正实时流式 ASR、原生透明浮窗和更可靠的上屏策略。
...
- 标题：`诊断工作台`。
- `运行状态`、`ASR 配置`、`测试操作` 和 `诊断日志` 面板。`运行状态` 里有 `全局快捷键`，用于确认快捷键注册是否成功，以及失败原因；还有 `桌面浮窗后端`，用于确认当前浮窗使用的是 WebView 方案还是后续原生方案。
...
- 按一次 `Ctrl+Alt+V` 时，主界面状态应进入录音；保持录音时，每隔几秒可能会出现“实验实时片段已上屏”日志，并把分段文本粘贴到当前光标位置；再按一次会停止录音并处理最后一段。
- 如果是 Tauri 桌面模式，诊断模式里会显示 `Tauri 运行中：可以验证系统能力`。
- 诊断模式 `桌面浮窗后端` 显示 `native-win32` 时，说明当前正在使用 Windows 原生浮窗；显示 `fallback-webview` 时，说明原生浮窗启动失败，已回退到 Tauri WebView 浮窗，具体原因会显示在同一行。
...
`Ctrl+Alt+V` 是切换录音模式：第一次按下开始录音，第二次按下停止。当前版本在这个模式里增加了实验分段实时输入。

使用方式：

1. 先把光标放到 Notepad、VS Code 输入框或浏览器输入框。
2. 按一次 `Ctrl+Alt+V`，底部浮窗进入录音状态。
3. 开始说话。应用会每约 4 秒读取一次正在录音缓冲区的新片段。
4. 如果 whisper.cpp 对这个片段返回文本，VoxType 会立即通过剪贴板把这段文本粘贴到当前光标处。
5. 再按一次 `Ctrl+Alt+V` 停止录音。停止时会尝试处理最后一段；如果录音过程中没有任何分段文本成功上屏，会回退到整段录音转写。

成功标志：

- 诊断日志出现 `实验实时输入已启动`。
- 录音过程中出现 `实验实时片段已上屏`。
- 目标输入框里陆续出现识别文本，而不是等到最后一次性出现。
- 停止后日志出现 `实验实时输入已停止` 或 `实验实时兜底上屏完成`。

失败或限制：

- 如果日志出现 `录音片段太短，暂不转写`，通常只是片段还不够 1 秒，继续说话即可。
- 如果 whisper.cpp 推理速度慢，文本会延迟出现；前端会避免多个 whisper.cpp 请求重叠。
- 这不是成熟输入法的真流式 partial。当前实现是“定时切片 -> whisper-cli.exe -> 剪贴板粘贴”，可能出现漏字、重复、上下文不足或标点不稳定。
- 当前仍使用剪贴板上屏，可能影响用户剪贴板。后续更可靠的方向是 SendInput 文本输入或 TSF。
...
1. 进入 `诊断模式`，点击 `测试桌面浮窗`。
2. 先看 `运行状态` 里的 `桌面浮窗后端`。如果显示 `native-win32`，说明当前使用 Windows 原生浮窗；如果显示 `fallback-webview`，说明原生浮窗启动失败，后面会带失败原因。
3. 如果屏幕底部没有出现小型五彩波纹浮窗，先看诊断日志：`桌面浮窗显示失败` 表示 Tauri overlay 窗口或透明窗口配置有问题；`桌面浮窗显示请求已发送` 但看不见，则优先怀疑窗口层级、多屏位置或系统透明窗口限制。
4. 如果后端是 `native-win32` 但黑色胶囊外侧仍能看到白色、灰色或浅色矩形边框，记录当前后端值和截图；这说明问题已经不是 WebView2 宿主窗口边框，需要继续排查原生窗口 region、GDI 绘制或系统合成设置。
5. 如果浮窗能显示且外侧没有白/灰矩形，点击 `隐藏桌面浮窗`，再聚焦 VS Code、Notepad 或浏览器输入框，按住 `Ctrl+Alt+Space`。
6. 回到诊断模式看日志：如果没有 `收到全局快捷键按下`，说明系统没有把快捷键事件交给 VoxType，可能是组合键被其他软件、远程桌面或系统占用。
7. 如果有 `收到全局快捷键按下/松开`，但没有文字上屏，说明热键已进入应用，继续看录音、转写和剪贴板上屏日志。
