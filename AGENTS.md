...
- 同一问题如果经过三轮修复、回归或真实测试仍未稳定解决，必须在 `docs/complicated-bugs/` 新增或更新一篇复杂 bug 文档，写清现象、错误假设、根因、最终修复、验证证据和后续经验。
...
- 涉及 UI、样式、设置面板、metadata、按钮、开关或记录列表展示的改动，必须先阅读并遵守 `docs/guide/ui-style-guide.md`；优先复用现有 class 和组件结构，新增 CSS 必须与当前浅绿产品化风格、字体、密度、圆角和响应式规则一致。
...
- On Windows, run repository commands through Git Bash by default: use bash -lc with the real command inside it.
- Do not use PowerShell for repository file edits, text replacement, search, diff, tests, builds, npm, cargo, git, or repo scripts.
- Use PowerShell only for Windows-only tasks such as registry, services, user-profile configuration, system APIs, or tools that only provide a .ps1 entry point.
- Never use PowerShell here-strings or PowerShell pipelines to edit UTF-8 Chinese files. For scripted edits, run Python, Node, Perl, sed, or awk from Git Bash and read/write UTF-8 explicitly.

## Testing Handoff Policy

- Before handing desktop dictation work to the maintainer, consult `docs/guide/testing-strategy.md` and run all low-risk automated checks that apply to the changed path.
- Verification reports must say which layer passed: automated checks, safe desktop smoke, manual desktop validation pending, or not verified.
- Do not use vague "tests passed" wording to imply that real microphone recording, real global shortcuts, real target windows, or system audio effects passed when they were not exercised.
- Automated tests must not send system global shortcuts, steal focus from real user windows, operate on IM/browser/elevated windows, modify audio effects, or save real microphone recordings unless the maintainer explicitly approves that controlled test.
...
- `docs/guide/ui-style-guide.md`：UI 风格指南；任何 UI、样式、设置项、metadata 或记录展示改动前必须参考。
- `docs/guide/testing-strategy.md`: layered testing strategy, safe desktop regression boundaries, and maintainer handoff reporting rules.
